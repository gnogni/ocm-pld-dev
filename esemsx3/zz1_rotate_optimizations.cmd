@echo off
rem --- 'zz1_rotate_optimizations.cmd' v3.2 by KdL (2025.06.29)
...
if not exist %SRC%%PROJECT%.qsf.balanced.extraeffort (
    ...
    if "%1"=="" color 5f&cls&echo.&echo [1] AREA ^& POWERPLAY OFF are set!
    del zz0_*.* >nul 2>nul
...
if not exist %SRC%%PROJECT%.qsf.area.off (
    ...
    if "%1"=="" color 4f&cls&echo.&echo [A] AREA ^& NORMAL COMPILATION are set!
    del zz0_*.* >nul 2>nul
...
if not exist %SRC%%PROJECT%.qsf.area.normal (
    ...
    if "%1"=="" color 6f&cls&echo.&echo [X] AREA ^& EXTRA EFFORT are set!
    del zz0_*.* >nul 2>nul
...
if not exist %SRC%%PROJECT%.qsf.area.extraeffort (
    ...
    if "%1"=="" color 2f&cls&echo.&echo [2] BALANCED ^& POWERPLAY OFF are set!
    del zz0_*.* >nul 2>nul
...
if not exist %SRC%%PROJECT%.qsf.balanced.off (
    ...
    if "%1"=="" color 3f&cls&echo.&echo [B] BALANCED ^& NORMAL COMPILATION are set!  ^(default^)
    del zz0_*.* >nul 2>nul
...
if not exist %SRC%%PROJECT%.qsf.balanced.normal (
    ...
    if "%1"=="" color 1f&cls&echo.&echo [Y] BALANCED ^& EXTRA EFFORT are set!
    del zz0_*.* >nul 2>nul
